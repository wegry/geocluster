...
class Cluster:
    ...
    def __str__(self):
        point_list = '\n'.split([str(point) for point in self.points])
...
class Point:
    ...
    def distance_between(self, other_point):
        ...
        a = sin(dlat / 2) ** 2 + cos(radians(self.lat)) * \
            cos(radians(other_point.lat)) * (sin(dlon / 2) ** 2)
...
class Geopoint(Point):
    ...
    def geocodeset(self):
        ...
        if self.address:
            ...
            self.lat = new_point[0]
            self.lon = new_point[1]
